    ...
    : AudioProcessorEditor (&p), audioProcessor (p), valueTreeState(vts)
{
	...
	for (int i = 0; i < N_SLIDERS_COUNT; i++)
	{
		...
		//Lable
		label.setText(BitCrusherAudioProcessor::paramsNames[i], juce::dontSendNotification);
		...
		m_sliderAttachment[i].reset(new SliderAttachment(valueTreeState, BitCrusherAudioProcessor::paramsNames[i], slider));
	}
